...
const colorsMap = new Map([
    ...
    ["440507", "red_nether_brick"], ["90552d", "redstone_lamp"], ["transparent", "white_stained_glass"]
]);
...
function* processColorData(player, test, startX, startY, startZ, width, height) {
    ...
    for (const value of Object.values(test)) {
        ...
        if (isHexColor || value[2] == "transparent") {
...
async function load(player, image, type) {
    try {
        const url = `http://localhost:8080/${type}`; // Change this to your api url
        const req = new HttpRequest(url);
        req.method = HttpRequestMethod.Post;
        req.body = image
        req.headers = [new HttpHeader("Content-Type", "application/json"), new HttpHeader("width", `${maxWidth}`), new HttpHeader("height", `${maxHeigth}`)];
        ...
        player.teleport({ x: startX + (data.width / 2), y: startY + 64, z: startZ }, {facingLocation: { x: startX + (data.width / 2), y: startY, z: startZ }})
        ...
        system.runTimeout(() => {system.runJob(processColorData(player, data.values, startX, startY, startZ, data.width, data.height))}, 200);
    } catch (error) {
