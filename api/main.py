...
def process_image_from_url():
    ...
    if image_url:
        ...
        if response.status_code == 200:
            ...
            if image.width > 256 or image.height > 256: # Modify these values ​​to increase or decrease the image size,
                image.thumbnail((256, 256))             # remember that whenever you increase the maximum image size, 
            elif image.width > 256:                     # it is recommended that you increase the tick distance from 
                w_percent = 256 / float(image.width)    # the server to avoid errors when setting the blocks.
                h_size = int(float(image.height) * float(w_percent))
                image = image.resize((256, h_size)) #
            elif image.height > 256:
                h_percent = 256 / float(image.height) #
                w_size = int(float(image.width) * float(h_percent))
                image = image.resize((w_size, 256)) #
